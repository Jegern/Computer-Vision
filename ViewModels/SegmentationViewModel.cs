...
public class SegmentationViewModel : ViewModel
{
    ...
    public SegmentationViewModel(ViewModelStore? store) : base(store)
    {
        ...
        VideoCringeCommand = new Command(
            VideoCringeCommand_OnExecuted,
            VideoCringeCommand_CanExecute);
    }
            ...
            while (flag)
            {
                ...
                {   
                    ...
                    while (flagFill)
                    {
                        ...
                        else if ((int) Tools.GetPixelIntensity(PictureBytes, (y + q + 1) * width * 4 + (x + w + 1) * 4) ==
                                 255)
                        {
                            Tools.SetPixel(
    ...
    #region VideoCringeCommand

    public Command? VideoCringeCommand { get; }

    private bool VideoCringeCommand_CanExecute(object? parameter) => !PictureSize.IsEmpty;

    private void VideoCringeCommand_OnExecuted(object? parameter)
    {
        const string fileName = @"C:/Users/sever/Documents/timer_video.mp4";

        var image = new Mat();
        var window = new Window("Kek");
